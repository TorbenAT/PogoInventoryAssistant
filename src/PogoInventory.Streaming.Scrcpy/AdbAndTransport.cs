...
public sealed class ScrcpyReadOnlyVideoTransport : IReadOnlyVideoTransport {
 readonly ScrcpyOptions _o; readonly AdbDeviceValidator _validator=new(); TcpClient? _tcp; Process? _server; Task<string>? _serverError; CancellationTokenSource? _run; int _state=(int)ComponentLifecycle.Created; public long TcpBytesReceived{get;private set;} public long EncodedPacketsPublished{get;private set;} public bool ServerProcessStarted{get;private set;} public bool AdbForwardCreated{get;private set;} public bool TcpSocketConnected{get;private set;} public int? ServerExitCode{get;private set;} public string? ServerStdErr{get;private set;} public TimeSpan? FirstByteLatency{get;private set;} public ComponentLifecycle Lifecycle=>(ComponentLifecycle)Volatile.Read(ref _state); ComponentLifecycle IReadOnlyVideoTransport.Lifecycle=>Lifecycle; public VideoStreamMetadata? Metadata{get;private set;}
 ...
 public async IAsyncEnumerable<EncodedVideoPacket> ReadPacketsAsync([EnumeratorCancellation]CancellationToken cancellationToken){
  ...
  await RequireAdbAsync($"-s {Q(_o.DeviceSerial)} forward tcp:{_o.LocalPort} localabstract:scrcpy",startup.Token,StreamFailureCode.VideoSocketConnectionFailed); AdbForwardCreated=true;
 ...
 public async ValueTask StopAsync(CancellationToken cancellationToken=default){var old=(ComponentLifecycle)Interlocked.Exchange(ref _state,(int)ComponentLifecycle.Stopping);if(old is ComponentLifecycle.Stopped or ComponentLifecycle.Disposed)return;_run?.Cancel();_tcp?.Dispose();if(_server is{HasExited:false}){try{_server.Kill(true);await _server.WaitForExitAsync(cancellationToken);}catch{}}try{if(_server is not null&&_server.HasExited){ServerExitCode=_server.ExitCode;ServerStdErr=_serverError is null?null:(await _serverError).Trim();}}catch{}try{await AdbDeviceValidator.RunAsync(_o.AdbPath,$"-s {Q(_o.DeviceSerial)} forward --remove tcp:{_o.LocalPort}",cancellationToken);}catch{} _server?.Dispose();_run?.Dispose();Volatile.Write(ref _state,(int)ComponentLifecycle.Stopped);}
