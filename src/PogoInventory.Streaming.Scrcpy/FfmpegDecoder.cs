...
public sealed class FfmpegBgraVideoFrameDecoder : IVideoFrameDecoder {
 readonly FfmpegDecoderOptions _o; Process? _p; public FfmpegBgraVideoFrameDecoder(FfmpegDecoderOptions options){_o=options;}
 public async IAsyncEnumerable<DecodedVideoFrame> DecodeAsync(IAsyncEnumerable<EncodedVideoPacket> packets,VideoStreamMetadata metadata,[EnumeratorCancellation]CancellationToken ct){
  ...
  try{if(!_p.Start())throw new StreamTransportException(new(StreamFailureCode.DecoderInitializationFailed,"FFmpeg did not start."));}catch(Exception e){throw new StreamTransportException(new(StreamFailureCode.DecoderInitializationFailed,"FFmpeg could not start.",e.Message));}
  ...
  try{while(!ct.IsCancellationRequested){var owner=MemoryPool<byte>.Shared.Rent(frameBytes);var mem=owner.Memory[..frameBytes];int read=0;var started=sw.Elapsed;while(read<frameBytes){var n=await _p.StandardOutput.BaseStream.ReadAsync(mem[read..],ct);if(n==0)break;read+=n;}if(read==0){owner.Dispose();break;}if(read!=frameBytes){owner.Dispose();throw new StreamTransportException(new(StreamFailureCode.FrameDecodeFailed,"FFmpeg returned a partial raw frame."));}var now=sw.Elapsed;yield return new(owner,frameBytes,width,height,stride,FramePixelFormat.Bgra32,++seq,now,DateTimeOffset.UtcNow,null,false,now-started);}}
  finally{try{await writer;}catch(OperationCanceledException){}if(_p is{HasExited:false})_p.Kill(true);}
 }
 public ValueTask DisposeAsync(){if(_p is{HasExited:false})_p.Kill(true);_p?.Dispose();return ValueTask.CompletedTask;}
}
