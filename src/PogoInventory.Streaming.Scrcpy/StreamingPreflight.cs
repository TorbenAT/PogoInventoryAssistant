...
public sealed record StreamingPreflightReport
{
    ...
    public string? ScrcpyServerSha256 { get; init; }
    ...
    public bool ReadOnlyConfiguration { get; init; }
...
public sealed class StreamingPreflightRunner
{
    public async Task<StreamingPreflightReport> RunAsync(StreamingPreflightOptions options, CancellationToken cancellationToken = default)
    {
        ...
        var serverVersionPassed = serverPassed && (string.IsNullOrWhiteSpace(options.ExpectedScrcpyServerVersion) || IsVersionIdentifiable(options.ScrcpyServerPath, options.ExpectedScrcpyServerVersion));
        ...
        if (device.Passed)
        {
            try
            {
                deviceModel = (await AdbDeviceValidator.RunAsync(options.AdbPath, $"-s \"{options.DeviceSerial}\" shell getprop ro.product.model", cancellationToken).ConfigureAwait(false)).Output.Trim();
