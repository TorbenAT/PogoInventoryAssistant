...
var tests = new (string Name, Func<Task> Run)[]
{
    ...
    ("Shared identity frame skips the redundant IV capture window", FrameDietSharedIdentityTests.SharedIdentityFrameSkipsRedundantIvWindowAsync),
    ("Fallback without a supplied identity capture still captures its own IV window", FrameDietSharedIdentityTests.FallbackWithoutIdentityCapturesOwnWindowAsync),
    ("Steady-state item costs exactly 7 captures", FrameDietSharedIdentityTests.SteadyStateItemCosts7CapturesAsync),
    ("Cleanup runner shares identity frames for IV and drops the ordinal>1 tag probe", FrameDietRunnerTests.RunAsync),
    ...
    ,("Late map settle is verified and does not fail the run", CleanupProofTests.LateMapSettleIsVerifiedAndDoesNotFailRunAsync)
    ,("Unsettled final map stays fail-closed after bounded re-observation", CleanupProofTests.UnsettledFinalMapStaysFailClosedAsync)
    ,("Late Details settle continues the exit chain", CleanupProofTests.LateDetailsSettleContinuesExitChainAsync)
    ,("Unsettled exit stays fail-closed after bounded re-observation", CleanupProofTests.UnsettledExitStaysFailClosedAsync)
    ,("Exit settle verification requires three consecutive Details frames", ExitSettleVerificationTests.RequiresThreeConsecutiveDetailsFramesAsync)
    ,("Exit settle verification is bounded by deadline when never settled", ExitSettleVerificationTests.IsBoundedByDeadlineWhenNeverSettledAsync)
    ...
    ,("Ground-truth CSV parsing preserves manual statuses", Sync(GroundTruthMeasurementTests.CsvParsingPreservesStatusesAndQuotedSources))
    ,("Ground-truth metrics classify field outcomes", Sync(GroundTruthMeasurementTests.FieldMetricSeparatesCorrectIncorrectUnknownAndUnverifiable))
    ,("Ground-truth review cause classification fails closed", Sync(GroundTruthMeasurementTests.ReviewCauseClassificationIsFailClosed))
    ...
    ,("Runner downgrades a Complete identity capture to Partial when CP is unknown", HeaderSemanticsCleanupTests.RunnerDowngradesCompleteIdentityWithUnknownCpToPartialAsync)
    ,("Shared status rule never promotes Unresolved to Partial", HeaderSemanticsCleanupTests.RunnerNeverPromotesUnresolvedAsync)
